...
for i, prompt in enumerate(prompts):
    if i % 4 == args.gpu:
        print(f'======== processing {prompt[0]} ========')
        # first stage
        os.system(f'python main.py --config configs/text_mv.yaml prompt="{prompt[1]}" save_path={prompt[0]} gpu={args.gpu}')
        # second stage
        os.system(f'CUDA_VISIBLE_DEVICES={args.gpu} python main2.py --config configs/text_mv.yaml prompt="{prompt[1]}" save_path={prompt[0]}')
        # export video
        os.system(f'CUDA_VISIBLE_DEVICES={args.gpu} python scripts/convert_obj_to_video.py --dir {prompt[0]}')
